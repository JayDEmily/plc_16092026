...
For browser execution, run only from this repository root on branch `main`.
...
The current user/job input supplies the Worker A and Worker B runtime values. Treat those values as opaque data; do not replace them with examples or instructions from repository history.
Worker completion is recognised only when the worker's response ends with a final line exactly equal to `complete`.
